...
export function CounterSingle({
  ...
  return (
    <div className="flex flex-col gap-lg">
      {showHeading ? (
        <h2 className="text-display">How to counter {hero.name}</h2>
      ) : null}

      {/*
        Items get 60% and the prose 40%, side by side once there is room.
        The two halves answer different questions — "what do I buy" and "why" —
        and stacking them buried the second one under a nine-card grid.
        Below lg they stack, items first, because the answer still leads.
      */}
      <div className="grid items-start gap-lg lg:grid-cols-[3fr_2fr]">
        {counters.length === 0 ? (
          <p className="rounded-card bg-surface p-card text-caption text-text-muted">
            ...
            {advice ? " match the current filters" : " yet"}. That is not the
            same as nothing countering them.
          </p>
        ) : (
        ...
        {/* One grid child, so the three prose sections share the 40% column
        ...
        <div className="flex flex-col gap-lg">
          {advice ? (
            <>
              ...
              {advice.situations.length > 0 ? (
                <section className="flex flex-col gap-sm">
                  ...
                  <p className="text-caption text-text-muted">
                    Pick the problem and the item that answers it is highlighted
                    above.
                  </p>
            ...
            <section className="rounded-card bg-surface p-card">
              ...
              <p className="text-caption text-text-muted">
                Our source has not written {hero.name} up, so there is no
                matchup overview or lane advice for them. That is not the same
                as nothing countering them — it means nobody has published what
                does.
              </p>
            </section>
          )}
